...
describe('Lancamento e2e test', () => {
    ...
    /* it('should create and save Lancamentos', async () => {
        ...
        expect(await lancamentoUpdatePage.getDataInput()).to.eq('2000-12-31', 'Expected data value to be equals to 2000-12-31');
        expect(await lancamentoUpdatePage.getDescricaoInput()).to.eq('descricao', 'Expected Descricao value to be equals to descricao');
        expect(await lancamentoUpdatePage.getValorInput()).to.eq('5', 'Expected valor value to be equals to 5');
        expect(await lancamentoUpdatePage.getUsuarioInput()).to.eq('usuario', 'Expected Usuario value to be equals to usuario');
        await lancamentoUpdatePage.save();
        expect(await lancamentoUpdatePage.getSaveButton().isPresent(), 'Expected save button disappear').to.be.false;

        expect(await lancamentoComponentsPage.countDeleteButtons()).to.eq(nbButtonsBeforeCreate + 1, 'Expected one more entry in the table');
    });*/
