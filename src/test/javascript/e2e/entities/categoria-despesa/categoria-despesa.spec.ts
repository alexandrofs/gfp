...
describe('CategoriaDespesa e2e test', () => {
    ...
    it('should create and save CategoriaDespesas', async () => {
        ...
        expect(await categoriaDespesaUpdatePage.getNomeInput()).to.eq('nome');
        expect(await categoriaDespesaUpdatePage.getUsuarioInput()).to.eq('usuario');
        await categoriaDespesaUpdatePage.save();
        expect(await categoriaDespesaUpdatePage.getSaveButton().isPresent()).to.be.false;

        expect(await categoriaDespesaComponentsPage.countDeleteButtons()).to.eq(nbButtonsBeforeCreate + 1);
    });
