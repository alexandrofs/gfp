...
describe('Despesa e2e test', () => {
    ...
    /* it('should create and save Despesas', async () => {
        ...
        expect(await despesaUpdatePage.getDataDespesaInput()).to.eq('2000-12-31', 'Expected dataDespesa value to be equals to 2000-12-31');
        expect(await despesaUpdatePage.getMesReferenciaInput()).to.eq('2000-12-31', 'Expected mesReferencia value to be equals to 2000-12-31');
        expect(await despesaUpdatePage.getDescricaoInput()).to.eq('descricao', 'Expected Descricao value to be equals to descricao');
        expect(await despesaUpdatePage.getValorInput()).to.eq('5', 'Expected valor value to be equals to 5');
        expect(await despesaUpdatePage.getUsuarioInput()).to.eq('usuario', 'Expected Usuario value to be equals to usuario');
        await despesaUpdatePage.save();
        expect(await despesaUpdatePage.getSaveButton().isPresent(), 'Expected save button disappear').to.be.false;

        expect(await despesaComponentsPage.countDeleteButtons()).to.eq(nbButtonsBeforeCreate + 1, 'Expected one more entry in the table');
    });*/
