...
describe('ContaPagamento e2e test', () => {
    ...
    it('should create and save ContaPagamentos', async () => {
        ...
        expect(await contaPagamentoUpdatePage.getNomeInput()).to.eq('nome', 'Expected Nome value to be equals to nome');
        expect(await contaPagamentoUpdatePage.getUsuarioInput()).to.eq('usuario', 'Expected Usuario value to be equals to usuario');
        await contaPagamentoUpdatePage.save();
        expect(await contaPagamentoUpdatePage.getSaveButton().isPresent(), 'Expected save button disappear').to.be.false;

        expect(await contaPagamentoComponentsPage.countDeleteButtons()).to.eq(
